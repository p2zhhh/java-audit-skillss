...
# Java API Parameter Chaining Fuzzer (Java 接口参数串联与实体绑定测试工具)
...
本技能通过自动化代码反编译、精准参数追踪、以及基于 JSON 实体的**强关联绑定（Entity Binding）**，实现了这种“参数串联”攻击路径的自动挖掘。它确保从同一个 JSON 对象中提取出的所有字段都被视为一个不可分割的实体（绑定在同一行），从而在后续 Fuzzing 中完美绕过复杂的业务逻辑校验。

## 🎯 何时调用此 Skill (When to Invoke)
- 用户要求：“帮我找找越权漏洞”、“有没有接口遍历风险”、“审计接口逻辑漏洞”、“测试一下这些参数能不能串起来查到别的信息”。
...
如果需要执行真实的动态 Fuzzing（DAST 模式），必须确保获取了目标 API 的基准 URL (Base URL)。
- **获取方式**：Agent 应首先检查用户的初始指令中是否已经提供了 URL。如果没有提供，Agent 必须通过对话框主动向用户提问（例如：“为了进行动态发包测试，请问目标环境的 Base URL 是多少？”）。
- 由于测试主要针对未授权暴露的接口进行逻辑探测，无需强制要求鉴权 Token 或自身 ID。
...
2. **【强制】生成并持久化攻击链路报告**: 向用户输出完整的漏洞报告，并**必须**使用文件写入工具将其追加保存到项目目录下的 `fuzzer_vulnerabilities.md` 文件中。
   - 指出哪个接口是受害者（Sink）。
   - 详细列出当时发送的具体参数（Payload）。
   - 必须将捕获到的敏感信息（如明文密码、身份证号）清晰地展示在报告中，作为漏洞存在的“铁证”。
