...
3. **业务逻辑阻断参数的智能伪造 (Mocking)**: 很多靶点接口除了需要 `userId`，还强制要求传入一些非 ID 类的业务参数才能往下走（例如：`POST /api/order/query` 需要 `{"userId": "1001", "startDate": "2023-01-01", "status": 1}`）。如果只传 `userId`，后端会直接报错“时间不能为空”。脚本无法猜测这些参数该填什么，但 Agent 可以根据参数名（`startDate`）和类型，智能伪造出合法的业务前置数据，从而成功“骗”过基础校验，直达越权逻辑。
